...
EOF
  ...
  ## purpose of the rewrite is to save gdrive and sdrive info and toss old GDSAs
      cat /pg/rclone/blitz.conf | grep -w "\[sd\]" -A 5 > /pg/var/sdrive.info
      cat /pg/rclone/blitz.conf | grep -w "\[gd\]" -A 4 > /pg/var/gdrive.info
      cat /pg/rclone/blitz.conf | grep -w "\[sc\]" -A 6 > /pg/var/scrypt.info
      cat /pg/rclone/blitz.conf | grep -w "\[gc\]" -A 6 > /pg/var/gcrypt.info
