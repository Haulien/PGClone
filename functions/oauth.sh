...
EOF
  ...
  curl --request POST --data "code=$token&client_id=$pgclonepublic&client_secret=$pgclonesecret&redirect_uri=urn:ietf:wg:oauth:2.0:oob&grant_type=authorization_code" https://accounts.google.com/o/oauth2/token > /opt/rclone/pgclone.info
...
########################
rm -rf /opt/rclone/.${type} 1>/dev/null 2>&1
echo "" > /opt/rclone/.${type}
echo "[$type]" >> /opt/rclone/.${type}
echo "client_id = $pgclonepublic" >> /opt/rclone/.${type}
echo "client_secret = $pgclonesecret" >> /opt/rclone/.${type}
echo "type = drive" >> /opt/rclone/.${type}
echo -n "token = {\"access_token\":${accesstoken}\"token_type\":\"Bearer\",\"refresh_token\":${refreshtoken}\"expiry\":\"${final}\"}" >> /opt/rclone/.${type}
echo "" >> /opt/rclone/.${type}
if [ "$type" == "sd" ] || [ "$type" == "sc" ]; then
teamid=$(cat /opt/rclone/pgclone.teamid)
echo "team_drive = $teamid" >> /opt/rclone/.sd; fi
echo ""

echo ${type} > /opt/rclone/oauth.check
oauthcheck
...
rm -rf /opt/rclone/.${entype} 1>/dev/null 2>&1
echo "" >> /opt/rclone/.${entype}
echo "[$entype]" >> /opt/rclone/.${entype}
echo "type = crypt" >> /opt/rclone/.${entype}
echo "remote = $type:/encrypt" >> /opt/rclone/.${entype}
echo "filename_encryption = standard" >> /opt/rclone/.${entype}
echo "directory_name_encryption = true" >> /opt/rclone/.${entype}
echo "password = $ENC_PASSWORD" >> /opt/rclone/.${entype}
echo "password2 = $ENC_SALT" >> /opt/rclone/.${entype};
fi
...
pgclonevars
  ...
  storage="/opt/rclone/teamdrive.output"
...
EOF
  ...
  if [[ "$token" = "exit" || "$token" == "Exit" || "$token" == "EXIT" ]]; then clonestart; fi
  curl --request POST --data "code=${token}&client_id=${pgclonepublic}&client_secret=${pgclonesecret}&redirect_uri=urn:ietf:wg:oauth:2.0:oob&grant_type=authorization_code" https://accounts.google.com/o/oauth2/token > /opt/var/token.part1
  curl -H "GData-Version: 3.0" -H "Authorization: Bearer $(cat /opt/var/token.part1 | grep access_token | awk '{ print $2 }' | cut -c2- | rev | cut -c3- | rev)" $gtype > $storage
...
if [[ $(cat /opt/rclone/teamdrive.name) == "" ]]; then
tee <<-EOF
